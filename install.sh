...
# udev rules so the peripheral-battery readout can talk to HID devices that
# expose no standard battery (Compx/Aula keyboard, HyperX headset) without root.
# Needs sudo; skipped if absent.
if compgen -G "$REPO_DIR/udev/*.rules" >/dev/null; then
  echo "==> Installing udev rules for peripheral batteries (sudo)"
  if sudo cp "$REPO_DIR"/udev/*.rules /etc/udev/rules.d/; then
